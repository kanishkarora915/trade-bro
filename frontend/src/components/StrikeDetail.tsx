...
export default function StrikeDetail({ chain, atm, onClose }: { chain: ChainRow[]; atm: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-tb-card border border-tb-border rounded-2xl p-5 max-w-5xl w-full max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
          ...
          <table className="w-full text-[10px] font-mono">
            <thead className="sticky top-0 bg-tb-card z-10">
              <tr className="text-tb-muted">
                <th colSpan={6} className="text-center text-neon-green py-1 border-b border-tb-border">— CALLS —</th>
                <th className="text-center py-1 border-b border-tb-border text-neon-cyan">STRIKE</th>
                <th colSpan={6} className="text-center text-neon-red py-1 border-b border-tb-border">— PUTS —</th>
              </tr>
